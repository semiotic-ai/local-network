...
BIN_CANDIDATES=(
  "/workspace/indexer-rs/target/${PROFILE}/indexer-tap-agent"
  "/workspace/indexer-rs/target/release/indexer-tap-agent"
  "/workspace/indexer-rs/target/debug/indexer-tap-agent"
...
if [ -z "${BIN:-}" ]; then
  echo "Error: indexer-tap-agent binary not found. Build it locally (e.g., 'cargo build --release -p indexer-tap-agent') and re-run."
  exit 1
fi

echo "Starting indexer-tap-agent using: $BIN"
